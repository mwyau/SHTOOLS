...
!------------------------------------------------------------------------------
!
!   Expand a complex, endpoint-including Clenshaw-Curtis grid into complex
...
!       1. Transform every latitude ring with a complex FFT.
!       2. Extract positive and negative longitudinal orders through
!          LMAX_CALC.
!       3. Continue each meridional order with the parity
!          f_m(2*pi-theta)=(-1)^m f_m(theta).
!       4. Zero-pad its meridional Fourier spectrum by two and inverse
...
!
!       IN
!           gridcc      Complex, non-extended grid of dimensions
!                       (L+2, 2*L+2).
!
!       OUT
!           cilm        Complex coefficients dimensioned at least
!                       (2, LMAX_CALC+1, LMAX_CALC+1). CILM(1,l,m) is the
!                       positive-order family and CILM(2,l,m) is the
!                       negative-order family before the (-1)^m factor.
...
!------------------------------------------------------------------------------
    ...
    real(dp), allocatable :: p(:), weights(:)
    complex(dp), allocatable :: row(:), spectrum(:), longitude(:,:), &
                              meridional(:,:), theta_input(:), &
                              theta_spectrum(:), theta_padded(:), &
                              theta_fine(:)
    ...
    allocate(p(np), weights(ntheta_quadrature+1), row(nlong), &
             spectrum(nlong), longitude(nlat, 2*lmax_comp+1), &
             meridional(ntheta_quadrature+1, 2*lmax_comp+1), &
    ...
    plan_long = fftw_plan_dft_1d(nlong, row, spectrum, FFTW_FORWARD, &
                                  FFTW_MEASURE)
    ...
    do i = 1, nlat
        ...
        call fftw_execute_dft(plan_long, row, spectrum)
        longitude(i, 1:lmax_comp+1) = spectrum(1:lmax_comp+1) / dble(nlong)
        do m = 1, lmax_comp
            longitude(i, lmax_comp+m+1) = spectrum(nlong-m+1) / dble(nlong)
        end do
    end do
    ...
    do m = 0, lmax_comp
        ...
        if (m > 0) then
            call CCInterpolate(longitude(:, lmax_comp+m+1), &
                               ntheta_coarse/2, m, plan_theta_forward, &
                               plan_theta_backward, theta_input, &
                               theta_spectrum, theta_padded, theta_fine, &
                               meridional(:, lmax_comp+m+1))
        end if
    end do
    call fftw_destroy_plan(plan_theta_forward)
    call fftw_destroy_plan(plan_theta_backward)
    ...
    do i = 1, ntheta_quadrature+1
        ...
        do m = 0, lmax_comp
            ...
            do l = m, lmax_comp
                idx = l*(l+1)/2 + m + 1
                cilm(1, l+1, m+1) = cilm(1, l+1, m+1) + weights(i) * &
                                      meridional(i, m+1) * p(idx)
                if (m > 0) then
                    cilm(2, l+1, m+1) = cilm(2, l+1, m+1) + &
                                          dble(signm)*weights(i) * &
                                          meridional(i, lmax_comp+m+1) * p(idx)
                end if
            end do
        end do
    end do
    ...
    do m = 0, lmax_comp
        do l = m, lmax_comp
            integral = CCPlmIntegral(l, m, lnorm, 1)
